...
if __name__ == "__main__":
    rospy.init_node("convert_2_to_2_1_db")

    print("Convert one Ades2DB to a new Ades2_1DB.")
    print("This process requires to run in parallel 2 dbs : 1 ades2db (original) and one ades2_1db (target)")
    ...
    default_namespace_orig = "orig"
    default_namespace_targ = "target"
    default_type_orig = 2
    default_type_targ = 2.1
    # target is always 2, we want to get rid of db1

    namespace_orig = raw_input("Enter namespace for original db (default:"+default_namespace_orig+"): ")
    type_orig = raw_input("Enter type (default:"+str(default_type_orig)+"): ")
    ...
    if not namespace_orig:
        namespace_orig = default_namespace_orig
    if not type_orig:
        type_orig = default_type_orig

    print("Waiting for original db to be up")
    srv_ades_orig = namespace_orig + "/ades2db" if type_orig == 2 else "/adesdb"
    rospy.wait_for_service(srv_ades_orig+"/list_ades")

    print("Waiting for target db to be up")
    if not namespace_targ:
        namespace_targ = default_namespace_targ
    srv_ades_targ = namespace_targ + "/ades2_1db"
    ...
    # Original services proxies
    list_ades_orig = rospy.ServiceProxy(srv_ades_orig+"/list_ades", ListAdes)
    get_preconds_orig = rospy.ServiceProxy(srv_ades_orig+"/get_preconds", GetAdesPreConds)
    get_effects_orig = rospy.ServiceProxy(srv_ades_orig+"/get_effects", GetAdesEffects)
    get_motions_orig = rospy.ServiceProxy(srv_ades_orig+"/get_motions", GetAdesMotions)
    ...
    ades_orig = list_ades_orig().ades_list
    len_orig = len(ades_orig)

    print(ades_orig)

    for a_s in ades_orig:
        # fetch infos:
        preconds_orig = get_preconds_orig(a_s)
        effects_orig = get_effects_orig(a_s)
        new_motion_seq = get_motions_orig(a_s)
        #print(new_pc)
        #print(new_ef)
        print(type(new_motion_seq))
        #print(new_motion_seq.motion_sequences)
        print(len(new_motion_seq.motion_sequences))
        for ms in new_motion_seq.motion_sequences:
            #print("--------------")
            #print(ms.input_types)
            for in_type in ms.input_types:
                new_in_type = KeyValPair()
                new_in_type.key = in_type
                new_in_type.value = ""
                ms.input_types2.append(new_in_type)
            
            print(ms.input_types2)

        print(new_motion_seq.motion_sequences)

        new_ades = AdesData()
        new_ades.ades_name = a_s
        
        new_ades.preconditions = preconds_orig.preconditions
        new_ades.effects = effects_orig.effects
        new_ades.motion_sequences = new_motion_seq.motion_sequences
