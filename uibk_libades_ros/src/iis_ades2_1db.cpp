...
#include "../include/iis_ades2_1db.h"
#include <boost/program_options.hpp>
#include <boost/filesystem.hpp>

using namespace ades;

Ades2_1db_ros::Ades2_1db_ros(ros::NodeHandle &nh, std::string home, int version):
    nh_(nh),
    database(home, version)
{

    ss_get_precond = nh_.advertiseService("ades2_1db/get_preconds", &Ades2_1db_ros::get_preconds_srv, this);
    ss_get_effect = nh_.advertiseService("ades2_1db/get_effects", &Ades2_1db_ros::get_effects_srv, this);
    ss_get_motion = nh_.advertiseService("ades2_1db/get_motions", &Ades2_1db_ros::get_motions_srv, this);
    ss_get_motion_sequence_name = nh_.advertiseService("ades2_1db/get_motion_sequence_names", &Ades2_1db_ros::get_motion_sequence_names_srv, this);
    ss_get_motion_name = nh_.advertiseService("ades2_1db/get_motion_names", &Ades2_1db_ros::get_motion_names_srv, this);


    ss_list_ades = nh_.advertiseService("ades2_1db/list_ades", &Ades2_1db_ros::list_ades_srv, this);
    ss_store_ades = nh_.advertiseService("ades2_1db/store_ades", &Ades2_1db_ros::store_ades_srv, this);
    ss_update_ades = nh_.advertiseService("ades2_1db/update_ades", &Ades2_1db_ros::update_ades_srv, this);
    ss_update_ades_motion = nh_.advertiseService("ades2_1db/update_ades_motion", &Ades2_1db_ros::update_ades_motion_srv, this);
    ss_delete_ades = nh_.advertiseService("ades2_1db/delete_ades", &Ades2_1db_ros::delete_ades_srv, this);
    ss_add_motion_sequence = nh_.advertiseService("ades2_1db/add_motion_sequence", &Ades2_1db_ros::add_motion_sequence_srv, this);


    ss_update_effect_models = nh_.advertiseService("ades2_1db/update_effect_models", &Ades2_1db_ros::update_effect_models_srv, this);
    ss_estimate_effect = nh_.advertiseService("ades2_1db/estimate_effect", &Ades2_1db_ros::estimate_effect_srv, this);
    ...
    db_changed = nh_.advertise<imagine_common::KeyValPair>("ades2_1db/db_updated", 1);

    shutdown = false;
}

Ades2_1db_ros::~Ades2_1db_ros()
{
}

bool Ades2_1db_ros::list_ades_srv(imagine_common::ListAdes::Request &rq, imagine_common::ListAdes::Response &rp)
{
...
bool Ades2_1db_ros::get_preconds_srv(imagine_common::GetAdesPreConds::Request &rq, imagine_common::GetAdesPreConds::Response &rp)
{
...
// Keep original ADES services for preconds and effects
bool Ades2_1db_ros::get_effects_srv(imagine_common::GetAdesEffects::Request &rq, imagine_common::GetAdesEffects::Response &rp)
{
...
// TODO
bool Ades2_1db_ros::get_motions_srv(imagine_common::GetAdesMotions::Request &rq, imagine_common::GetAdesMotions::Response &rp)
{
    ...
    fail_mo_seq_.input_types2 = std::vector<imagine_common::KeyValPair>();
    ...
    if(database.isInDB(rq.ades_name))
    {
        ...
        for(auto sequence : ades.getMotionSequences())
        {
            ...
            // (sequence_name, input_types2, effect_prob, effect_pred, motions)
            imagine_common::MotionSequence mo_seq_;
            mo_seq_.sequence_name = sequence.first;
            // getInputTypes on ADES2_1 returns a std::map<std::string, std::string>, manual conversion needed
            std::vector<imagine_common::KeyValPair> input_types_vector;
            //auto input_types_pairs = sequence.second.getInputTypes()
            for(auto pair : sequence.second.getInputTypes())
            {
                imagine_common::KeyValPair kv_pair;
                kv_pair.key = pair.first;
                kv_pair.value = pair.second;
                input_types_vector.push_back(kv_pair);
            }
            mo_seq_.input_types2 = input_types_vector;
...
// TODO
bool Ades2_1db_ros::get_motion_names_srv(imagine_common::GetAdesMotionNames::Request &rq, imagine_common::GetAdesMotionNames::Response &rp)
{
...
bool Ades2_1db_ros::get_motion_sequence_names_srv(imagine_common::GetAdesMotionSequenceNames::Request &rq, imagine_common::GetAdesMotionSequenceNames::Response &rp)
{
...
bool Ades2_1db_ros::store_ades_srv(imagine_common::StoreAdes::Request &rq, imagine_common::StoreAdes::Response &rp)
{
    ...
    if( !(database.isInDB(rq.ades.ades_name)) )
    {
        ...
        std::map<std::string, MotionSequence2_1> newMotionSequences;
        for(auto ms : rq.ades.motion_sequences)
        {
            //MotionSequence ms_;
            MotionSequence2_1 ms2_;
            ms2_.setScore(ms.score);
            std::map<std::string, std::string> inputTypes;
            
            for(auto it : ms.input_types2)
            {
                //inputTypes.push_back(it);
                std::pair<std::string, std::string> new_input_pair;
                new_input_pair.first = it.key;
                new_input_pair.second = it.value;
                inputTypes.insert(new_input_pair);
            }
            ...
            for(auto motion : ms.motions)
            {
                ...
                // In any case, we add the name to the Ades2_1 motion sequence
                if( found_motion.name.empty() )
                {
                    // motion not found
            ...
            newMotionSequences.insert(std::pair<std::string, MotionSequence2_1>(ms.sequence_name, ms2_));
        }
        ...
        Ades2_1 newAdes(rq.ades.ades_name, newPC, newEF, newMotionSequences);
...
bool Ades2_1db_ros::update_ades_srv(imagine_common::UpdateAdes::Request &rq, imagine_common::UpdateAdes::Response &rp)
{
        ...
        if( !rq.ades.motion_sequences.empty() )
        {
            ...
            for(auto ms : rq.ades.motion_sequences)
            {
                ...
                for(auto motion : ms.motions)
                {
                    ...
                    // In any case, we add/update the name to the Ades2_1 motion sequence
                    if( found_motion.name.empty() )
                    {
                        // motion not found
                        imagine_common::StoreMotion store_motion_call;
...
bool Ades2_1db_ros::add_motion_sequence_srv(imagine_common::AddMotionSequence::Request &rq, imagine_common::AddMotionSequence::Response &rp)
{
...
bool Ades2_1db_ros::update_ades_motion_srv(imagine_common::UpdateAdesMotionNames::Request &rq, imagine_common::UpdateAdesMotionNames::Response &rp)
{
    ...
    if(!rq.ades_name.empty())
    {
        ...
        if(!rq.sequence_name.empty() & adesExists)
        {
            ...
            MotionSequence2_1 * target_motionsequence = target_ades->modifyMotionSequence(rq.sequence_name);
            int count = 0;
...
bool Ades2_1db_ros::delete_ades_srv(imagine_common::DeleteAdes::Request &rq, imagine_common::DeleteAdes::Response &rp)
{
...
//ss_update_effect_models = nh_.advertiseService("adesdb/update_effect_models", &Ades2_1db_ros::update_effect_models_srv, this);
bool Ades2_1db_ros::update_effect_models_srv(imagine_common::UpdateEffects::Request &rq, imagine_common::UpdateEffects::Response &rp)
{
...
bool Ades2_1db_ros::estimate_effect_srv(imagine_common::EstimateEffect::Request &rq, imagine_common::EstimateEffect::Response &rp)
{
    rp.value = 0.0;
    rp.probability = -1;
...
int main(int argc, char** argv)
{
    ...
    //init the ROS node
    ros::init(argc, argv, "ades2_1db_node");
    ros::NodeHandle nh;
    ...
    Ades2_1db_ros ros_database(nh, vm["home"].as<std::string>(), vm["version"].as<int>());
